...
async function getForm(slug: string): Promise<FormWithFields | null> {
  ...
  const { data } = await supabase
    ...
    .single();
  if (!data) return null;
  // Only expose active fields to public — archived fields are for historical
  // admin views only and must never appear on the live submission form.
    ...
    <div className="min-h-screen bg-ocean-950 flex flex-col">
      ...
      <header className="border-b border-ocean-800/60 bg-ocean-900/80 backdrop-blur-sm">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-3">
          <a href="/" className="text-teal-400 hover:text-teal-300 transition-colors text-sm font-medium">
            OMSP
          </a>
          <span className="text-ocean-700">/</span>
          <span className="text-slate-400 text-sm truncate">{form.title}</span>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-6 py-12 lg:py-16">
        ...
        <div className="mb-10">
          ...
          {form.description && (
            <p className="mt-4 text-slate-300 leading-relaxed">{form.description}</p>
          )}
          {form.deadline && (
            <div className="mt-4 flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${expired ? "bg-red-400" : "bg-teal-400"}`} />
              <span className={`text-sm font-mono ${expired ? "text-red-400" : "text-slate-400"}`}>
                {expired
        ...
        {expired ? (
          <div className="glass-card p-12 text-center">
            ...
            <h2 className="font-display text-xl font-bold text-white mb-2">This form is closed</h2>
            <p className="text-slate-400 text-sm">The deadline has passed. Check back for future opportunities.</p>
            <a href="/opportunities" className="btn-ghost inline-flex mt-6 text-sm px-6 py-2.5">
