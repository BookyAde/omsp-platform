...
// Icon map — inline SVGs keyed by icon name from constants
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex flex-col fixed left-0 top-0 bottom-0 w-64
                        bg-ocean-900 border-r border-ocean-800/60 z-40">
        ...
        <nav className="flex-1 py-6 px-3 space-y-0.5 overflow-y-auto" aria-label="Admin navigation">
          {ADMIN_NAV.map((item) => {
            ...
            return (
              ...
              >
                <span className={cn("flex-shrink-0", active ? "text-teal-400" : "text-slate-500")}>
                  {ICONS[item.icon]}
                </span>
                {item.label}
              </Link>
        ...
        <div className="p-4 border-t border-ocean-800/60">
          <Link
            href="/"
            target="_blank"
            className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-slate-500
                       hover:text-slate-300 hover:bg-ocean-800/40 transition-colors text-sm"
          >
